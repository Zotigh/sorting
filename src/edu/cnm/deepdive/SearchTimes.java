...
public class SearchTimes {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < haystack.length; i++) {
      haystack[i] = rng.nextInt(10_000);
    }
    for (int i = 0; i < needles.length; i++) {
      needles[i] = rng.nextInt(10_000);
    }
    long startTime;
    long endTime;
    ...
    System.out.printf("Time elapsed for linear search: %.3f seconds%n",
        (endTime - startTime) / 1000d); //the d is for decimal interger

  //  MergeSorter sorter = new MergeSorter();
    startTime = System.currentTimeMillis();
    Arrays.sort(haystack);
    endTime = System.currentTimeMillis();
    System.out.printf("Time elapsed for MergeSort search: %.3f seconds%n",
        (endTime - startTime) / 1000d); //the d is for decimal interger
